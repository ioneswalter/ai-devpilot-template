...
-- 3. Create invoices table (only if referenced Prisma-managed tables exist)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'escrow_payments')
     AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'additional_work_escrows')
  ...
  THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'invoices' AND policyname = 'invoices_provider_select') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'invoices' AND policyname = 'invoices_service_insert') THEN
      CREATE POLICY invoices_service_insert ON invoices
        FOR INSERT
        WITH CHECK (true);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'invoices' AND policyname = 'invoices_service_update') THEN
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'service_providers')
     AND NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'objects' AND policyname = 'invoices_storage_provider_select') THEN
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'objects' AND policyname = 'invoices_storage_admin_select') THEN
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'objects' AND policyname = 'invoices_storage_service_insert') THEN
    CREATE POLICY invoices_storage_service_insert ON storage.objects
      FOR INSERT
      WITH CHECK (bucket_id = 'invoices');
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'objects' AND policyname = 'invoices_storage_service_update') THEN
    CREATE POLICY invoices_storage_service_update ON storage.objects
      FOR UPDATE
      USING (bucket_id = 'invoices');
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'objects' AND policyname = 'invoices_storage_service_delete') THEN
