...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'marketplace_posts')
...
-- Trigger to auto-update updated_at (safe outside DO block - uses IF NOT EXISTS pattern via DROP/CREATE)
