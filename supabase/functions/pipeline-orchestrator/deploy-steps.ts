...
function isTransient(msg: string): boolean {
  return ['ECONNRESET', 'timeout', 'ECONNREFUSED', '503', '429', 'overloaded'].some((t) =>
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      ...
      if (attempt < MAX_RETRIES - 1 && isTransient(msg)) {
        fixAttempts.push({
          description: `Transient retry ${attempt + 1}`,
          status: 'failed',
          error: msg,
      ...
      fixAttempts.push({
        description: `Final attempt ${attempt + 1}`,
        status: 'failed',
        error: msg,
        timestamp: new Date().toISOString(),
      });
...
export async function deployFunction(
  ...
  const entrypoint =
    files.find((f) => f.file_path.endsWith('index.ts'))?.file_path ?? files[0]?.file_path;
    ...
    try {
      ...
      if (resp.ok) {
        return {
          ...
          duration_ms: Date.now() - start,
          error: null,
          details: `Deployed ${files.length} file(s)`,
          started_at: startedAt,
          completed_at: new Date().toISOString(),
          retry_count: attempt,
          fix_attempts: fixAttempts,
      ...
      if (attempt < MAX_RETRIES - 1 && (resp.status >= 500 || resp.status === 429)) {
        fixAttempts.push({
          description: `Transient retry ${attempt + 1} (HTTP ${resp.status})`,
          status: 'failed',
          error: body,
          timestamp: new Date().toISOString(),
        });
        await appendLog(
          ...
          pipelineId,
          'warn',
          `Function deploy retry ${attempt + 1}: ${resp.status}`
        );
        await new Promise((r) => setTimeout(r, Math.pow(2, attempt) * 1000));
        continue;
      }
      ...
      if (attempt < MAX_RETRIES - 1 && isTransient(msg)) {
        fixAttempts.push({
          description: `Network retry ${attempt + 1}`,
          status: 'failed',
          error: msg,
          timestamp: new Date().toISOString(),
        });
