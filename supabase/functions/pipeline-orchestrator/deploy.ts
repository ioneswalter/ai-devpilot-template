...
import { completeDeploy, failDeploy, type DeployResults } from './deploy-lifecycle.ts';
  ...
  try {
    ...
    if (conflicts.length > 0) {
      ...
      await appendLog(
        ...
        pipelineId,
        'warn',
        `File conflicts detected: ${conflicts.length} file(s) overlap with other pipelines`
      );
    }
    ...
    if (!lockAcquired) {
      ...
      await appendLog(
        ...
        pipelineId,
        'error',
        'Failed to acquire deployment slot — pipeline may have been cancelled or timed out'
      );
    ...
    const migrations = files.filter(
      (f) => f.file_path.includes('migrations/') && f.file_path.endsWith('.sql')
    );
    ...
    if (migrations.length === 0 && edgeFunctions.length === 0) {
      await appendLog(
        ...
        pipelineId,
        'info',
        'No server-side artifacts — deployment skipped'
      );
      await completeDeploy(supabase, pipelineId, requestId, results);
      return;
    }
      ...
      for (const mig of sorted) {
        ...
        if (result.status === 'failed') {
          ...
          await appendLog(
            ...
            pipelineId,
            'error',
            `Migration failed: ${mig.file_path} — ${result.error}`
          );
          await failDeploy(supabase, pipelineId, requestId, results);
          return;
        }
    ...
    if (edgeFunctions.length > 0) {
      ...
      await appendLog(
        ...
        pipelineId,
        'info',
        `Deploying ${slugs.length} Edge Function(s)...`
      );

      for (const slug of slugs) {
        const fnFiles = edgeFunctions.filter((f) =>
          f.file_path.startsWith(`supabase/functions/${slug}/`)
        );
        const result = await deployFunction(slug, fnFiles, pipelineId, supabase);
        results.functions.push(result);
        if (result.status === 'failed') {
          results.overall_status = results.migrations.some((m) => m.status === 'success')
            ? 'partial'
            : 'failed';
          await appendLog(
            supabase,
            pipelineId,
            'error',
            `Function deploy failed: ${slug} — ${result.error}`
          );
          await failDeploy(supabase, pipelineId, requestId, results);
          return;
        }
