...
export const featureApiMethods = {
  ...
  getCurrencyConfigs: () =>
    apiClient<{
      data: { configs: import('@/features/admin/CurrencyConfigPanel').CurrencyConfigRow[] };
    }>('admin-dashboard?section=currency_config', { method: 'GET' }),
  ...
  updateUser: (userId: string, updates: Record<string, unknown>) =>
    apiClient<{ data: { user_id: string; updated: boolean } }>(
      'admin-dashboard?section=update_user',
      {
        method: 'POST',
  ...
  blockUser: (userId: string, blocked: boolean) =>
    apiClient<{ data: { user_id: string; is_blocked: boolean } }>(
      'admin-dashboard?section=block_user',
      {
        method: 'POST',
  ...
  getLmsOverview: (params?: { category?: string; search?: string }) => {
    ...
    if (params?.search) qs.set('search', params.search);
    return apiClient<{ data: LmsOverviewData }>(`admin-dashboard?${qs.toString()}`, {
      method: 'GET',
    });
  },

  getLmsCourseDetail: (courseId: string) =>
    apiClient<{ data: LmsCourseDetailData }>(
      `admin-dashboard?section=lms_course_detail&course_id=${courseId}`,
      { method: 'GET' }
    ),

  getLmsUserProgress: (enrollmentId: string) =>
    apiClient<{ data: LmsUserProgressData }>(
      `admin-dashboard?section=lms_user_progress&enrollment_id=${enrollmentId}`,
      { method: 'GET' }
    ),

  // Spec Artifacts (FR-104)
  getSpecArtifacts: (featureId: string) =>
    apiClient<{ data: { artifacts: SpecArtifact[] } }>(
      `admin-dashboard?section=spec_artifacts&feature_id=${featureId}`,
      { method: 'GET' }
    ),
  ...
  getCourseDetail: (courseId: string) =>
    apiClient<{ data: { course: unknown; modules: unknown[] } }>(
      `admin-dashboard?section=course_detail&course_id=${courseId}`,
      { method: 'GET' }
    ),

  saveCourse: (data: Record<string, unknown>) =>
    apiClient<{ data: { course: { id: string; title: string; status: string } } }>(
      'admin-dashboard?section=course_save',
      {
        method: 'POST',
  ...
  saveModule: (data: Record<string, unknown>) =>
    apiClient<{ data: { module: { id: string; title: string; sort_order: number } } }>(
      'admin-dashboard?section=module_save',
      {
        method: 'POST',
        body: JSON.stringify(data),
      }
  ...
  reorderModules: (courseId: string, moduleIds: string[]) =>
    apiClient<{ data: { reordered: boolean } }>('admin-dashboard?section=module_reorder', {
      method: 'POST',
      body: JSON.stringify({ course_id: courseId, module_ids: moduleIds }),
    }),

  getQuizQuestions: (moduleId: string) =>
    apiClient<{ data: { questions: unknown[] } }>(
      `admin-dashboard?section=quiz_questions&module_id=${moduleId}`,
      { method: 'GET' }
    ),
  ...
  saveQuiz: (moduleId: string, questions: unknown[]) =>
    apiClient<{ data: { saved: number } }>('admin-dashboard?section=quiz_save', {
      method: 'POST',
      body: JSON.stringify({ module_id: moduleId, questions }),
    }),

  getTemplateList: () =>
    apiClient<{ data: { templates: unknown[] } }>('admin-dashboard?section=template_list', {
      method: 'GET',
    }),
  ...
  generateOutline: (topic: string, category?: string) =>
    apiClient<{ data: { modules: unknown[]; model: string } }>(
      'course-ai?section=generate_outline',
      {
        method: 'POST',
        body: JSON.stringify({ topic, category, module_count_hint: 5 }),
      }
    ),

  generateQuiz: (moduleId: string, content: string, questionCount = 5) =>
    apiClient<{ data: { questions: unknown[]; model: string } }>(
      'course-ai?section=generate_quiz',
      {
        method: 'POST',
        body: JSON.stringify({ module_id: moduleId, content, question_count: questionCount }),
      }
    ),
  ...
  moderateContent: (content: string, moduleTitle: string) =>
    apiClient<{ data: { warnings: unknown[] } }>('course-ai?section=moderate_content', {
      method: 'POST',
      body: JSON.stringify({ content, module_title: moduleTitle }),
    }),
  ...
  getImplementation: (featureId: string) =>
    apiClient<{ data: ImplementationRequestWithItems }>(
      `implement-feature?feature_id=${featureId}`
    ),
  ...
  implementNextTask: (requestId: string, fileContexts?: Record<string, string>) =>
    apiClient<{ data: { done: boolean; remaining: number; task_id?: string; status?: string } }>(
      'implement-feature?action=implement',
      {
        method: 'POST',
        body: JSON.stringify({
          request_id: requestId,
          ...(fileContexts ? { file_contexts: fileContexts } : {}),
        }),
      }
    ),
  ...
  fixBuildErrors: (data: { errors: BuildErrorItem[]; files: FileContentItem[]; stage?: string }) =>
    apiClient<{ data: { fixes: FixResultItem[]; error_count: number } }>(
      'implement-feature?action=fix-errors',
      {
        method: 'POST',
        body: JSON.stringify(data),
      }
...
export interface ComplexityScore {
  ...
  factors: Record<string, ComplexityFactorResult>;
  scored_at: string;
}
