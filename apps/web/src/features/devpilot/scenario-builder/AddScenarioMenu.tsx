...
export function AddScenarioMenu({ conversationId, onAdd, isAdding }: AddScenarioMenuProps) {
  ...
  return (
    <div className="relative inline-block text-left">
      ...
      {open && (
        <div
          className="absolute right-0 mt-1 z-10 w-44 rounded border border-gray-200 bg-white shadow-md py-1"
          role="menu"
        >
          <button
            type="button"
            role="menuitem"
            onClick={() => choose('happy_path')}
            className="w-full text-left text-xs px-3 py-1.5 hover:bg-emerald-50 text-emerald-700"
          >
            Happy path
          </button>
          <button
            type="button"
            role="menuitem"
            onClick={() => choose('edge_case')}
            className="w-full text-left text-xs px-3 py-1.5 hover:bg-amber-50 text-amber-700"
          >
            Edge case
          </button>
        </div>
