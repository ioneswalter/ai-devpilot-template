  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => {
        if (data)
          setSections(
            [...new Set(data.map((r: { spec_section: string }) => r.spec_section))].sort()
          );
      });
  }, []);
  return sections;
...
async function mergeIntoInFlightFeature(
  targetCode: string,
  throwawayId: string,
  f: ProposalFormState,
  conversationId: string
): Promise<UpdatedFeatureRow & { in_flight_version_label: string }> {
...
async function deleteThrowawayFeature(featureId: string): Promise<void> {
  ...
  const r3 = await supabase
    ...
    .eq('submitted_feature_id', featureId);
  if (r3.error)
    throw new Error(`Cleanup failed (clearing submitted_feature_id): ${r3.error.message}`);
...
async function mergeIntoVersionedFeature(
  targetCode: string,
  throwawayId: string,
  f: ProposalFormState,
  conversationId: string
): Promise<UpdatedFeatureRow> {
...
async function replaceFeatureContent(
  targetCode: string,
  throwawayId: string,
  f: ProposalFormState,
  conversationId: string
  ...
  const { data: updated, error: updateErr } = await supabase
    ...
    .update({
      title: f.title,
      description: f.description,
      acceptance_criteria: criteriaFromForm(f),
      priority: f.priority,
      status: 'proposed',
  ...
  if (updateErr || !updated)
    throw new Error(`Failed to update ${targetCode}: ${updateErr?.message ?? 'no data returned'}`);
...
export function ProposalPanel({
  ...
  const handleSubmitAll = async () => {
    ...
    setError(null);
    const unsubmitted = forms
      .map((f, i) => ({ form: f, index: i }))
      .filter(({ form: f }) => !f.submitted);
    ...
    for (const { form: f, index } of unsubmitted) {
      ...
      if (hasJourneys) {
        proposalData.journeys = f.journeys.map((j) => ({
          ...
          acceptance_scenarios: j.acceptance_scenarios
            .split('\n')
            .map((s: string) => s.trim())
            .filter(Boolean),
        }));
      ...
      try {
        const result = (await devpilotApi.submitProposal(conversationId, proposalData))
          .data as SubmitProposalResponse;
  ...
  const handleDedupDecision = async (decisions: DedupDecision[]) => {
    ...
    const useExisting = decisions.find(
      (d) =>
    ...
    try {
      if (mergeInFlight) {
        const updated = await mergeIntoInFlightFeature(
          mergeInFlight.feature_code,
          dedupResult.feature.id,
          f,
          conversationId
        );
        ...
        onSubmitted([
          {
            ...
            feature: {
              ...updated,
              status: updated.status as 'proposed',
      ...
      if (versionBump) {
        const updated = await mergeIntoVersionedFeature(
          versionBump.feature_code,
          dedupResult.feature.id,
          f,
          conversationId
        );
      ...
      if (useExisting) {
        const updated = await replaceFeatureContent(
          useExisting.feature_code,
          dedupResult.feature.id,
          f,
          conversationId
        );
    ...
    <div className="flex flex-col h-full bg-white border-l">
      {dedupResult && (
        <DeduplicationWarning
          ...
          onSubmitAnyway={handleDedupDecision}
      ...
      {!allSubmitted && (
        <div className="border-t px-4 py-3">
          <button
            ...
            disabled={
              isSubmitting ||
              forms.some((f) => !f.submitted && (!f.title.trim() || !f.description.trim()))
            }
