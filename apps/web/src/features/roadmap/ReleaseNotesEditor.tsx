...
const SaveIcon = ({ className = '' }: { className?: string }) => (
  <svg className={className} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg>
)
const WandIcon = ({ className = '' }: { className?: string }) => (
  <svg className={className} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 4-1.17 2.34L11.5 7.5l2.33 1.16L15 11l1.17-2.34L18.5 7.5l-2.33-1.16z"/><path d="m2 22 10-10"/><path d="m12 12 4-4"/></svg>
)
...
export function ReleaseNotesEditor({
  ...
  const saveNotesMutation = useMutation({
    mutationFn: (notesContent: string) =>
      updateRelease({ id: releaseId, notes: notesContent }),
    onSuccess: () => {
  ...
  const renderMarkdown = useCallback((text: string) => {
    return text
      ...
      .map((line, i) => {
        ...
        if (line.startsWith('### ')) {
          return <h3 key={i} className="text-lg font-medium mb-2">{line.slice(4)}</h3>
        }
        if (line.startsWith('- ')) {
        ...
        if (line.startsWith('> ')) {
          return <blockquote key={i} className="border-l-4 border-gray-300 pl-4 italic mb-2">{line.slice(2)}</blockquote>
        }
        if (line.match(/`([^`]+)`/)) {
          return <p key={i} className="mb-2" dangerouslySetInnerHTML={{
            __html: line.replace(/`([^`]+)`/g, '<code class="bg-gray-100 px-1 py-0.5 rounded text-sm">$1</code>')
          }} />
        }
        if (line.trim() === '') {
          return <br key={i} />
          ...
          <div className="flex items-center gap-2">
            ...
            {featureIds.length === 0 && (
              <span className="text-sm text-gray-500">
                Select features to generate notes
              </span>
            )}
          </div>
        ...
        <div className="grid w-full grid-cols-2 mb-4 bg-gray-100 rounded-lg p-1">
          <button
            ...
            className={`flex items-center justify-center gap-2 px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${
              activeTab === 'edit' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-600 hover:text-gray-900'
            }`}
          >
