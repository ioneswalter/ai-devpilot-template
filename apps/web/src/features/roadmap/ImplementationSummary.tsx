...
export function ImplementationSummary({ implementedCount, failedCount, totalAccepted, codeApplied }: ImplementationSummaryProps) {
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <h4 className={`text-sm font-semibold ${
            ...
            {codeApplied ? 'Code Applied to Project' :
          ...
          <p className="text-xs text-gray-600 mt-1">
            ...
            {failedCount > 0 && !codeApplied && ` ${failedCount} skipped (too large — can be split manually later).`}
          </p>
          {!codeApplied && !allFailed && (
            <div className="mt-3 space-y-1.5">
              ...
              <ul className="text-xs text-gray-600 space-y-1 list-disc list-inside">
                ...
                <li>Click <strong>Write Code</strong> to apply the {implementedCount} completed tasks</li>
                {failedCount > 0 && (
...
function StatusIcon({ isSuccess, allFailed }: { isSuccess: boolean; allFailed: boolean }) {
  ...
  return (
    <svg className={`w-5 h-5 mt-0.5 flex-shrink-0 ${isSuccess ? 'text-green-600' : 'text-amber-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
