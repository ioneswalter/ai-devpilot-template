    ...
    <>
      {currentCycle > 1 && (
        <div className="px-4 py-2 bg-blue-50 border-b border-blue-100 text-xs text-blue-700">
          Re-review — submitting will record cycle {currentCycle}. Previous cycle decisions appear
          under each item.
        </div>
      )}
        ...
        {pkg.prototype_refs?.length > 0 && (
          <div className="bg-indigo-50 border border-indigo-100 rounded-lg p-3 mb-2">
            ...
            <p className="text-xs text-indigo-600">
              {pkg.prototype_refs.length} prototype(s) linked. Visual references appear alongside
              criteria.
            </p>
          </div>
        )}
