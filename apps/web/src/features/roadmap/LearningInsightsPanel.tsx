...
function PatternRow({ pattern }: { pattern: FailurePattern }) {
  ...
  const color = pattern.frequency >= 5 ? 'bg-red-400' : pattern.frequency >= 3 ? 'bg-amber-400' : 'bg-blue-400';
  return (
...
function RecommendationCard({ rec, onApprove, onDismiss, isActing }: {
    ...
    <div className="border rounded-lg p-2.5 bg-white space-y-1.5">
      ...
      {rec.status === 'pending' && (
        <div className="flex gap-1.5 pt-1">
          ...
          <button onClick={onDismiss} disabled={isActing}
...
export function LearningInsightsPanel() {
  ...
  const { data: insights } = useQuery({
    ...
    queryFn: async () => { const r = await adminApi.getLearningInsights(); return r.data; },
  ...
  return (
    <div className="border rounded-lg bg-indigo-50 border-indigo-200 p-3 space-y-2">
      <button onClick={() => setExpanded(v => !v)}
        ...
        <span className="font-semibold text-indigo-800 uppercase tracking-wider">Learning Insights</span>
        <span className="flex items-center gap-2 text-indigo-600">
          ...
          {pendingRecs.length > 0 && (
            <span className="px-1.5 py-0.5 bg-amber-100 text-amber-700 rounded-full text-[10px] font-bold">{pendingRecs.length} pending</span>
          )}
        </span>
      </button>
      ...
      {expanded && (
        <div className="space-y-3 pt-1">
          {insights.top_patterns.length > 0 && (
            <div>
              <h5 className="text-[10px] font-semibold text-indigo-600 uppercase tracking-wider mb-1">Top Patterns</h5>
              <div className="space-y-0.5">
                {insights.top_patterns.slice(0, 5).map(p => <PatternRow key={p.id} pattern={p} />)}
              </div>
            </div>
          )}
          ...
          {insights.recommendations.length > 0 && (
            <div>
              <h5 className="text-[10px] font-semibold text-indigo-600 uppercase tracking-wider mb-1">Recommendations</h5>
