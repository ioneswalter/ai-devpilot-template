...
export function BuildCheckPanel({
  ...
  const fixErrors = useCallback(
    async (failedStage: StageResult, completed: StageResult[]) => {
      ...
      try {
        ...
        if (!fixes?.length) {
          setState({
            phase: 'fix-failed',
            message: 'AI could not determine fixes',
            failedStage,
            completed,
          });
          return;
    ...
    <div className="rounded-lg border p-4 space-y-3">
      ...
      {state.phase === 'running' && (
        <StatusRow
          icon={<SpinnerIcon />}
          color="blue"
          text={`Running ${STAGE_LABELS[state.stage]}...`}
        />
      )}
      ...
      {state.phase === 'fix-failed' && (
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => fixErrors(state.failedStage, state.completed)}
    ...
    <div className="flex gap-1">
      {stages.map((stage) => {
        ...
        return (
          <div
            key={stage}
            className={`flex-1 px-2 py-1 rounded text-xs font-medium text-center ${bg}`}
          >
            {running && (
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <span className="text-xs font-medium text-red-700">
          {errors.length} {STAGE_LABELS[stage]} error{errors.length !== 1 ? 's' : ''} in {fileCount}{' '}
          file{fileCount !== 1 ? 's' : ''}
        </span>
        <div className="flex gap-2">
          <button
...
async function runStage(stage: PipelineStage, writtenFiles?: string[]): Promise<StageResult> {
  ...
  const body =
    stage !== 'typecheck' && writtenFiles?.length ? JSON.stringify({ files: writtenFiles }) : '{}';
  ...
  try {
    ...
    return {
      stage,
      ok: false,
      errors: [{ file: '', line: 0, col: 0, code: stage, message: `Failed to run ${stage}` }],
    };
  }
}
...
function StatusRow({ icon, color, text }: { icon: React.ReactNode; color: string; text: string }) {
  ...
  return (
    <div
      className={`flex items-center gap-2 px-3 py-2 rounded-md text-xs font-medium ${colors[color] ?? ''}`}
    >
      {icon}
      {text}
    </div>
...
const WandIcon = () => (
  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    <path
      ...
      strokeWidth={2}
      d="m15 4-1.17 2.34L11.5 7.5l2.33 1.16L15 11l1.17-2.34L18.5 7.5l-2.33-1.16z"
    />
