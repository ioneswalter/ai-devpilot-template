...
export function AutomatedExecuteView({
  ...
  const handleUpdateRecommendation = useCallback(
    async (id: string, status: 'accepted' | 'dismissed' | 'deferred') => {
      try {
  ...
  const triggerImprovementAnalysis = useCallback(
    async (sr: BrowserSuiteResult) => {
      setLoadingRecs(true);
      try {
        const apiTimings = sr.results
          .filter((r) => r.steps_total === 1 && r.result === 'passed')
          .map((r) => ({
            test_case_id: r.test_case_id,
            endpoint: r.test_case_title,
            duration_ms: r.duration_ms,
          }));
        const e2eTimings = sr.results
          .filter((r) => r.steps_total > 1)
          .map((r) => ({
            test_case_id: r.test_case_id,
            step_timings: (r.step_results || []).map((s) => ({
              step: s.step_number,
    ...
    async (script: ScriptListItem) => {
      ...
      if (script.tier === 'api') {
        ...
        try {
          ...
          const failureReason =
            apiResult.result !== 'passed'
              ? apiResult.assertions.find(
                  (a: { passed: boolean; description: string }) => !a.passed
                )?.description || 'API assertion failed'
              : undefined;
          ...
          setSingleResults((prev) => ({
            ...
            [script.id]: {
              result: 'error',
              duration_ms: 0,
              failure_reason: err instanceof Error ? err.message : 'API test execution failed',
            },
          }));
        }
  ...
  return (
    <div className="flex flex-col h-full">
      <AutomatedExecuteHeader
        ...
        extensionChecking={ext.checking}
        extensionAvailable={ext.isAvailable}
        connectionError={ext.connectionError ?? null}
        phase={phase}
        submitError={submitError}
      />

      <div
        className={`flex-1 min-h-0 p-4 ${phase === 'running' ? 'flex flex-col' : 'overflow-y-auto space-y-3'}`}
      >
        ...
        {phase === 'preflight-failed' && auto.preflightResult && (
          <PreflightReport
            ...
            onRetry={async () => {
              const r = await auto.runPreflightCheck(featureCode);
              if (r.passed) await executeAfterPreflight();
            }}
            ...
            {auto.scripts.length === 0 && (
              <div className="text-center py-6 space-y-3">
                ...
                <p className="text-xs text-gray-400">
                  Run{' '}
                  <CopyableCommand
                    command={`\\generate-tests ${featureCode}`}
                    className="bg-gray-100"
                  />{' '}
                  in Claude Code to generate scripts from actual source code.
                </p>
                <button
                  onClick={onSwitchToManual}
      ...
      <AutomatedExecuteFooter
        ...
        markedCount={markedCount}
        extensionAvailable={ext.isAvailable}
        hasScripts={auto.scripts.length > 0}
        allPassed={allPassed}
      />
    </div>
  );
