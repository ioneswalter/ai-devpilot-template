          ...
          {diff.acceptance_criteria.added.length > 0 && (
            <div className="mt-1 space-y-0.5">
              {diff.acceptance_criteria.added.map((c, i) => (
                <p
                  key={`a-${i}`}
                  className="text-xs text-green-700 bg-green-50 pl-3 border-l-2 border-green-400 py-0.5 rounded-r"
                >
                  + {c}
                </p>
              ))}
          ...
          {diff.acceptance_criteria.removed.length > 0 && (
            <div className="mt-1 space-y-0.5">
              {diff.acceptance_criteria.removed.map((c, i) => (
                <p
                  key={`r-${i}`}
                  className="text-xs text-red-700 bg-red-50 pl-3 border-l-2 border-red-400 py-0.5 rounded-r line-through"
                >
                  - {c}
                </p>
              ))}
    ...
    <div>
      ...
      {changed ? (
        <div className="mt-1 grid grid-cols-2 gap-2">
          <p className="text-xs text-red-700 bg-red-50 p-2 rounded border border-red-200 line-through">
            {oldValue}
          </p>
