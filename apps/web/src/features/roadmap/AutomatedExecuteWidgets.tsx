        ...
        return (
          <div key={script.id}>
            ...
            >
              ...
              <span
                className={`text-xs font-medium text-gray-800 flex-1 ${isExpanded ? '' : 'truncate'}`}
              >
              ...
              {script.generation_notes?.includes('[GATE:WARN:') && (
                <span
                  ...
                  title={script.generation_notes
                    .split('\n')
                    .filter((l) => l.includes('[GATE:WARN:'))
                    .join('; ')}
                >
                  gate warning
                </span>
    ...
    >
      ...
      <div className="flex gap-4 text-xs">
        <span className="text-green-700">
          {result.api_results.passed + result.e2e_results.passed} passed
        </span>
        {result.api_results.failed + result.e2e_results.failed > 0 && (
          <span className="text-red-700">
            {result.api_results.failed + result.e2e_results.failed} failed
          </span>
        )}
        {result.api_results.errors + result.e2e_results.errors > 0 && (
          <span className="text-red-700">
            {result.api_results.errors + result.e2e_results.errors} errors
          </span>
        )}
        {result.e2e_results.skipped_stale > 0 && (
          <span className="text-amber-700">{result.e2e_results.skipped_stale} skipped (stale)</span>
        )}
...
export function TestCaseResultRow({
  ...
  const statusLabel =
    result === 'passed' ? 'Passed' : result === 'failed' ? 'Failed' : (skippedReason ?? 'Skipped');
