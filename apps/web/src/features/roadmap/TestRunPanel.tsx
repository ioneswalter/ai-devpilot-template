...
export function TestRunPanel({
  ...
  const skippedCount = testCases.filter(
    (tc) => tc.passed == null && tc.id && lastRunResults[tc.id] === 'skipped'
  ).length;
  const notRunCount = testCases.filter((tc) => tc.passed == null).length - skippedCount;
  const allPassed =
    testCases.length > 0 && failedCount === 0 && notRunCount === 0 && skippedCount === 0;
    ...
    queryFn: async () => {
      try {
        ...
        return {
          ...
          pending: tasks.filter((t: { decision: string }) => t.decision === 'pending').length,
          rejected: tasks.filter((t: { decision: string }) => t.decision === 'rejected').length,
        };
      } catch {
  ...
  const buildNeedsReview =
    featureStatus === 'in_development' && (buildPending > 0 || buildRejected > 0);
  ...
  const needsBuild =
    featureStatus === 'proposed' || featureStatus === 'reviewed' || featureStatus === 'specified';
  if (needsBuild)
    return (
      <BuildRequiredGate
          ...
          <div className="px-4 py-3 bg-purple-50 border-b border-purple-100 shrink-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-sm font-medium text-purple-900">
                New version needs test generation
              </span>
            </div>
            <p className="text-xs text-purple-700">
              The tests below are from {versionInfo.priorLabel} (passed). Run{' '}
              <CopyableCommand
                command={`\\generate-tests ${featureCode}`}
                className="bg-purple-100"
              />{' '}
              in Claude Code to generate tests for the {versionInfo.currentLabel} delta criteria.
            </p>
          </div>
        )}
        <TestRunStatusView
  ...
  return (
    <TestRunManualView
      ...
      onEnvironmentChange={setEnvironment}
      results={results}
      onResultChange={(id, r) => setResults((prev) => ({ ...prev, [id]: r }))}
      notes={notes}
      onNotesChange={(id, n) => setNotes((prev) => ({ ...prev, [id]: n }))}
      lastRunResults={lastRunResults}
      submitError={exec.submitError}
      isSubmitting={exec.isSubmitting}
