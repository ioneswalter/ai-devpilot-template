  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      ...
      <div className="flex-1 bg-gray-100 rounded-full h-1.5">
        <div className={`h-1.5 rounded-full ${barColor}`} style={{ width: `${Math.max(4, score)}%` }} />
      </div>
      <span className="w-8 text-right text-gray-500 font-mono">{score}</span>
      <span className="text-gray-400 truncate max-w-[140px]" title={detail}>{detail}</span>
    </div>
  );
}

export function ComplexityScorePanel({ score, taskItems, parentTaskId }: ComplexityScorePanelProps) {
                ...
                {childTasks.map((child, i) => (
                  <div key={child.id} className="flex items-center gap-1.5 text-xs text-gray-600">
                    ...
                    <span className={`w-1.5 h-1.5 rounded-full ${
                      child.implementation_status === 'completed' ? 'bg-green-400' :
                      child.implementation_status === 'failed' ? 'bg-red-400' : 'bg-gray-300'
                    }`} />
