...
const sectionLabels: Record<string, string> = {
  criterion: 'Acceptance Criteria', test_case: 'Test Cases',
  edge_case: 'Edge Cases', description: 'Description Enhancements',
};

export function SpecReviewPanel({
  featureId, featureCode, featureTitle, featureStatus, onClose, onReviewComplete,
  ...
  if (!spec.isLoading && !spec.review) {
    return (
      <SpecReviewStartView
        ...
        needsProposalReview={needsProposalReview} needsSpecGeneration={needsSpecGeneration}
        isStarting={spec.isStarting} startError={spec.startError}
        onStartReview={() => spec.startReview()} onArtifactsLoaded={setArtifactCount} onClose={onClose}
      />
    );
  }
  ...
  if (showSuccess) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-3 p-8">
        <div className={`w-14 h-14 rounded-full flex items-center justify-center ${showSuccess === 'approved' ? 'bg-green-100' : 'bg-amber-100'}`}>
        ...
        <h4 className="text-base font-semibold text-gray-900">{showSuccess === 'approved' ? 'Spec Approved' : 'Sent Back for Revision'}</h4>
        <p className="text-sm text-gray-500 text-center">
          {showSuccess === 'approved' ? `${spec.acceptedCount} items added to ${featureCode}` : `Feedback sent — ${featureCode} returned to ideation`}
        </p>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 border-b bg-white">
        <div className="flex items-center gap-2 mb-1">
          ...
          {spec.isReviewActive && spec.pendingCount > 0 && (
            <button onClick={() => spec.acceptAll()} disabled={spec.isUpdating}
        ...
        <div className="flex items-center gap-2 text-xs text-gray-500">
          ...
          <span className={`px-1.5 py-0.5 rounded font-medium ${spec.review?.status === 'in_review' ? 'bg-blue-100 text-blue-700' : spec.review?.status === 'approved' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
          ...
          {(() => {
            ...
            return label ? <span className="ml-1 px-2 py-0.5 text-xs font-medium rounded-full bg-purple-100 text-purple-700">{label}</span> : null;
          })()}
          {!spec.items.some(i => i.source === 'ai_generated') && spec.isReviewActive && (
            <span className="bg-amber-50 text-amber-600 border border-amber-200 px-1.5 py-0.5 rounded">Manual review mode</span>
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        {sectionOrder.map(type => {
          ...
          return (
            <div key={type}>
              <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">{sectionLabels[type]} ({items.length})</h4>
              <div className={spec.isReviewActive ? 'space-y-2' : 'space-y-1'}>
                {items.map(item => <ReviewItemCard key={item.id} item={item} isReviewActive={spec.isReviewActive} onDecision={spec.updateItemDecision} onComment={spec.addComment} />)}
              </div>
            </div>
          );
        })}
        ...
        {spec.isReviewActive && (
          <div className="border-t pt-4">
            ...
            <div className="flex gap-2 mb-2">
              <select value={newItemType} onChange={(e) => setNewItemType(e.target.value as ReviewItemType)} className="text-xs border rounded px-2 py-1.5 bg-white">
                <option value="criterion">Criterion</option><option value="test_case">Test Case</option><option value="edge_case">Edge Case</option>
              </select>
              <input type="text" value={newItemContent} onChange={(e) => setNewItemContent(e.target.value)} placeholder="Enter new item..."
                className="flex-1 text-sm border rounded px-2 py-1.5 focus:ring-1 focus:ring-blue-300 focus:border-blue-300"
                onKeyDown={(e) => { if (e.key === 'Enter' && newItemContent.trim()) { spec.addItem(newItemType, newItemContent.trim()); setNewItemContent(''); } }} />
              <button onClick={() => { if (newItemContent.trim()) { spec.addItem(newItemType, newItemContent.trim()); setNewItemContent(''); } }}
                disabled={!newItemContent.trim() || spec.isUpdating}
                className="px-3 py-1.5 text-xs font-medium bg-gray-100 text-gray-700 rounded hover:bg-gray-200 disabled:opacity-40 disabled:cursor-not-allowed">Add</button>
            </div>
          </div>
        )}
        ...
        {spec.history.length > 1 && (
          <div className="border-t pt-4">
            <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Previous Reviews</h4>
      ...
      <ReviewApprovalBar pendingCount={spec.pendingCount} acceptedCount={spec.acceptedCount} rejectedCount={spec.rejectedCount}
        isReviewActive={spec.isReviewActive} isApproving={spec.isApproving} isSendingBack={spec.isSendingBack}
        onApprove={() => spec.approveReview().then(() => { setShowSuccess('approved'); setTimeout(() => onReviewComplete(), 1500); }).catch(() => {})}
        onSendBack={(fb) => spec.sendBack(fb).then(() => { setShowSuccess('sent_back'); setTimeout(() => onReviewComplete(), 1500); }).catch(() => {})}
        onClose={onClose} />
    </div>
  );
}
