...
export function useExtensionBridge() {
  ...
  const captureEvidence = useCallback(
    async (stepNumber: number, targetUrl?: string): Promise<CaptureEvidenceResult | null> => {
      const resp = await sendToExtension(
        'CAPTURE_EVIDENCE',
        { step_number: stepNumber, target_url: targetUrl },
        8000
      );
      if (resp.success && resp.data) {
  ...
  const getStoredEvidence = useCallback(async (): Promise<CaptureEvidenceResult | null> => {
  ...
  const executeTestScript = useCallback(
    async (steps: ScriptStep[], baseUrl?: string): Promise<TestScriptExecutionResult | null> => {
      ...
      console.log(
        '[SpecKit Bridge] executeTestScript called with',
        steps.length,
        'steps, timeout:',
        timeoutMs
      );
      ...
      console.log(
        '[SpecKit Bridge] executeTestScript response:',
