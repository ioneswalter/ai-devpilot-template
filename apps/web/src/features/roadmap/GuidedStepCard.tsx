...
import type { GuidedStep, StepEvidence } from './guided-testing-types';
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          {step.requires_navigation && (
            <p className="mt-1 text-[10px] text-amber-600 bg-amber-50 inline-block px-1.5 py-0.5 rounded">
              Navigate to: {step.requires_navigation}
            </p>
          )}
        </div>
      </div>
        ...
        <div className="mt-3 pt-2 border-t border-gray-100">
          ...
          {extensionAvailable && (
            <button
              onClick={onCapture}
              disabled={capturing}
              className="mb-2 w-full text-xs py-1.5 rounded border border-indigo-200 text-indigo-700 bg-indigo-50 hover:bg-indigo-100 disabled:opacity-50 transition-colors"
            >
