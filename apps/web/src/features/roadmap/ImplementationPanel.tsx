...
export function ImplementationPanel({
  ...
  const specReviewQuery = useQuery({
    ...
    queryFn: async () => {
      try {
        return (await adminApi.getReview(featureId)).data.review;
      } catch {
        return null;
      }
    },
    ...
    if (featureStatus === 'in_development') {
      return (
        <ManualImplGate
          ...
          onStartAnyway={() => impl.requestImplementation().catch(() => {})}
          onClose={onClose}
        />
      );
    }
    ...
    <div className="flex flex-col h-full">
        ...
        <div className="flex items-center gap-2 text-xs text-gray-500">
          ...
          <span
            className={`px-1.5 py-0.5 rounded font-medium ${
              impl.request?.status === 'completed' && impl.pendingCount > 0
                ? 'bg-amber-100 text-amber-700'
                : impl.request?.status === 'completed'
                  ? 'bg-green-100 text-green-700'
                  : impl.request?.status === 'failed'
      ...
      {versionInfo && (
        <div className="px-4 py-3 bg-purple-50 border-b border-purple-100">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm font-medium text-purple-900">
              {featureStatus === 'specified'
                ? 'New version ready to build'
                : featureStatus === 'in_testing'
                  ? 'Version built — ready to deploy'
                  : featureStatus === 'in_development'
                    ? 'Version build in progress'
                    : 'New version'}
            </span>
          </div>
          <p className="text-xs text-purple-700">
            {featureStatus === 'specified' ? (
              <>
                Run <CopyableCommand command={`\\build ${featureCode}`} className="bg-purple-100" />{' '}
                in Claude Code to implement the {versionInfo.currentLabel} delta.
              </>
            ) : featureStatus === 'in_testing' ? (
              <>
                The {versionInfo.currentLabel} build is complete. Run{' '}
                <CopyableCommand command={'\\deploy'} className="bg-purple-100" /> to deploy changes
                to production.
              </>
            ) : (
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <VersionGroupedTasks
          ...
          onDecision={(id, data) => impl.updateTaskItem(id, data)}
          onComment={(id, comment) => impl.updateTaskItem(id, { comment })}
        ...
        <CompliancePanel
          featureId={featureId}
          taskItems={impl.taskItems}
          onBlockedChange={handleComplianceBlocked}
        />
