...
function tokenize(content: string): LineToken[] {
  ...
  for (const line of lines) {
    ...
    if (/^\d+\.\s/.test(line.trimStart())) {
      ...
      if (match) {
        tokens.push({ type: 'ol', num: match[1], text: match[2] });
        continue;
      }
    }
    if (line.trimStart().startsWith('> ')) {
      tokens.push({ type: 'blockquote', text: line.trimStart().slice(2) });
      continue;
    }
...
function renderInline(text: string): ReactNode[] {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (match[2]) {
      ...
      parts.push(
        <code key={key++} className="px-1 py-0.5 bg-gray-100 text-pink-600 rounded text-[11px]">
          {match[5]}
        </code>
      );
    } else if (match[6]) {
      parts.push(
        <span key={key++} className="font-semibold text-indigo-600">
          {match[6]}
        </span>
      );
    }
...
export function MarkdownRenderer({ content }: MarkdownRendererProps) {
  ...
  while (i < tokens.length) {
    ...
    if (t.type === 'h1') {
      elements.push(
        <h2 key={i} className="text-sm font-bold text-gray-900 mt-4 mb-2 pb-1 border-b">
          {renderInline(t.text)}
        </h2>
      );
    } else if (t.type === 'h2') {
      elements.push(
        <h3 key={i} className="text-xs font-bold text-gray-800 mt-3 mb-1.5 uppercase tracking-wide">
          {renderInline(t.text)}
        </h3>
      );
    } else if (t.type === 'h3') {
      elements.push(
        <h4 key={i} className="text-xs font-semibold text-gray-700 mt-2 mb-1">
          {renderInline(t.text)}
        </h4>
      );
    } else if (t.type === 'h4') {
      elements.push(
        <h5 key={i} className="text-xs font-medium text-gray-600 mt-1.5 mb-1">
          {renderInline(t.text)}
        </h5>
      );
    } else if (t.type === 'hr') {
      ...
      while (
        j < tokens.length &&
        (tokens[j].type === 'table_row' || tokens[j].type === 'table_sep')
      ) {
        if (tokens[j].type === 'table_sep') {
          headerDone = true;
          j++;
          continue;
        }
                  ...
                  .map((row, ri) => (
                    <tr key={ri} className="bg-gray-50">
                      {row.cells.map((cell, ci) => (
                        <th
                          key={ci}
                          className="px-2 py-1.5 text-left font-semibold text-gray-600 border border-gray-200"
                        >
                          {renderInline(cell)}
                        </th>
                      ))}
      ...
      elements.push(
        <pre
          key={i}
          className="my-2 p-2 bg-gray-900 text-green-300 rounded text-[11px] font-mono overflow-x-auto leading-relaxed"
        >
          {codeLines.join('\n')}
        </pre>
      );
      ...
      elements.push(
        <p key={i} className="text-xs text-gray-700 py-0.5 leading-relaxed">
          {renderInline(t.text)}
        </p>
      );
    }
