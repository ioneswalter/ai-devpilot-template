    ...
    <div className="border border-indigo-200 rounded-lg overflow-hidden">
      ...
      {!ext.checking && !ext.isAvailable && (
        <div className="px-3 py-2 bg-amber-50 border-b border-amber-100 text-xs text-amber-700">
          ...
          <button onClick={() => window.location.reload()} className="ml-1 underline font-medium">
            Refresh
          </button>
        </div>
      )}
        ...
        <div className="px-3 py-2 bg-indigo-50 border-b border-indigo-100">
          ...
          <div className="mt-1 w-full bg-indigo-200 rounded-full h-1">
            <div
              ...
              style={{
                width: `${(auto.executingProgress.current / auto.executingProgress.total) * 100}%`,
              }}
            />
          </div>
        </div>
      ...
      <div className="divide-y">
        {auto.scripts.length === 0 && !auto.generating && (
          <div className="px-3 py-4 text-center text-xs text-gray-500">
            No automated scripts yet. Run{' '}
            <CopyableCommand command={'\\generate-tests FR-XXX'} className="bg-gray-100" /> in
            Claude Code to create them from actual source code.
          </div>
        )}
          ...
          return (
            <ScriptRow
              ...
              onRun={async () => {
                ...
                const result = await auto.executeScript(
                  script,
                  'development',
